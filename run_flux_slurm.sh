#!/usr/bin/env bash
#SBATCH --job-name=flux-train
##SBATCH --output=logs/flux.%j.out
#SBATCH --nodes=1
...
CONTAINER_NAME_PREFIX=${CONTAINER_NAME_PREFIX:-"mlperf"}
...
CONTAINER_WORKDIR=${CONTAINER_WORKDIR:-"/zirui/code/torchtitan"}
...
srun --ntasks="${NNODES}" --ntasks-per-node=1 bash -s -- "$@" <<EOF
set -ex
...
docker_env_args=(
    ...
    -e PYTHONPATH="/zirui/code/mlperf-common"
)
